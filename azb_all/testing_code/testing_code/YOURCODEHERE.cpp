...
int validateConfiguration(std::string configuration) {
	...
	width = 8 * pow(2, extractConfigParam(configuration, 0));
	L1dblockSize = 8 * (1 << extractConfigParam(configuration, 2));
    L1iblockSize = 8 * (1 << extractConfigParam(configuration, 2));
    L2blocksize = 16 << extractConfigParam(configuration, 8);
...
std::string generateNextConfigurationProposal(std::string currentconfiguration,
	...
	while (!validateConfiguration(nextconfiguration) ||
		...
		// if (best){
		// 	bestIndex[currentlyExploringDim] = extractConfigParam(bestConfig, currentlyExploringDim);
		// } 
		...
		// 	ss << extractConfigParam(bestConfig, EXPLORE[dim]) << " ";
		// }
		...
		for (int i = 0;
				i < (NUM_DIMS - NUM_DIMS_DEPENDENT); ++i) {
					if (doneDim[i] == 1){
						...
						ss << extractConfigParam(bestConfig, i) <<" ";
						// cout << "Adding best dim " << " " <<ss.str()<<endl;
					} else {
						if (i == currentlyExploringDim){
							// check start from 0 since baseline does not
							if (doneDim[i] == -1 && extractConfigParam(currentconfiguration, i) != 0){
							ss << "0 ";
							doneDim[i] = 0;
							} else {
								// increment value by 1
								int nextValue = extractConfigParam(nextconfiguration, i) + 1;
							...
							ss << extractConfigParam(currentconfiguration, i) << " ";
